...
export default function DabRenewalFinalReport({ companyId = 'default' }: Props) {
  ...
  useEffect(() => {
    const appUnsubscribe = onSnapshot(doc(db, 'companies', companyId, 'dabRenewalApplications', 'current'), snapshot => {
      if (snapshot.exists()) setApplication(snapshot.data() as DabRenewalApplication);
    });
