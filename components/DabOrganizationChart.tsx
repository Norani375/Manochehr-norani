...
function NodeCard({ node, onEdit }: { node: OrgNode; onEdit: (node: OrgNode) => void }) {
  return <li className="relative flex flex-col items-center">
    <button type="button" onClick={() => onEdit(node)} className="w-56 border-2 border-slate-700 bg-white p-4 text-center shadow-sm transition hover:bg-slate-50 print:shadow-none">
      <strong className="block text-sm">{node.title}</strong><span className="mt-1 block text-xs text-slate-600">{node.subtitle}</span>
    </button>
    {node.children?.length ? <><div className="h-6 border-r-2 border-slate-500" /><ul className="flex flex-wrap justify-center gap-4 border-t-2 border-slate-500 pt-6">{node.children.map((child) => <NodeCard key={child.id} node={child} onEdit={onEdit} />)}</ul></> : null}
  </li>;
}
  ...
  return <main dir="rtl" className="min-h-screen bg-slate-100 p-4 md:p-8 print:bg-white print:p-0">
    <div className="mx-auto max-w-7xl">
      <header className="mb-5 border-2 border-slate-800 bg-white p-5 text-center print:border-x-0 print:border-t-0">
        ...
        <h1 className="mt-3 text-xl font-bold">تشکیلاتی چارت شرکت صرافی و خدمات پولی</h1>
        <p className="mt-1 text-sm">{companyName}</p>
      </header>

      <div className="mb-5 grid gap-3 border bg-white p-4 md:grid-cols-3 print:hidden">
        <label className="text-sm">نام شرکت<input value={companyName} onChange={(e) => setCompanyName(e.target.value)} className="mt-1 w-full border p-2" /></label>
        <label className="text-sm">تاریخ نافذ شدن<input type="date" value={effectiveDate} onChange={(e) => setEffectiveDate(e.target.value)} className="mt-1 w-full border p-2" /></label>
      ...
      <section className="mt-5 border-2 border-slate-700 bg-white p-5 print:break-inside-avoid">
        ...
        <div className="grid grid-cols-1 gap-0 md:grid-cols-3"><div className="min-h-24 border p-4 text-sm">رئیس هیئت نظارتی<br /><br />امضا: __________________</div><div className="min-h-24 border p-4 text-sm">مدیر عمومی<br /><br />امضا: __________________</div><div className="min-h-24 border p-4 text-sm">مهر شرکت<br /><br />__________________</div></div>
