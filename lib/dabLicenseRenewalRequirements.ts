...
export const DAB_LICENSE_RENEWAL_REQUIREMENTS: RenewalDocumentRequirement[] = [
  {
    key: 'originalLicense',
    titleFa: 'اصل جواز فعالیت',
    required: true,
    legalBasis: 'ماده ۱۵، فقره (۱)، جزء ۱',
    descriptionFa: 'اصل جواز فعالیت برای بررسی و ثبت پرونده تمدید.',
  },
  {
    key: 'renewalFeeReceipt',
    titleFa: 'سند پرداخت فیس درخواست تمدید جواز',
    required: true,
    legalBasis: 'ماده ۱۵، فقره (۱)، جزء ۲',
    descriptionFa: 'رسید یا سند معتبر پرداخت فیس درخواست تمدید.',
  },
  {
    key: 'taxPaymentOrClearance',
    titleFa: 'رسید پرداخت مالیات یا تصدیق عدم باقی‌داری مالیاتی',
    required: true,
    legalBasis: 'ماده ۱۵، فقره (۱)، جزء ۳',
    descriptionFa: 'یکی از اسناد معتبر پرداخت مالیات یا تصدیق عدم باقی‌داری مالیاتی.',
  },
  {
    key: 'criminalClearanceOwner',
    titleFa: 'تصدیق عدم مسئولیت جنایی مالک / سهمداران مربوط',
    required: true,
    legalBasis: 'ماده ۱۵، فقره (۱)، جزء ۴',
    descriptionFa: 'تصدیق از مراجع ذیصلاح مطابق الزامات قابل تطبیق.',
  },
  {
    key: 'criminalClearanceEmployees',
    titleFa: 'تصدیق عدم مسئولیت جنایی کارمندان رسمی',
    required: true,
    legalBasis: 'ماده ۱۵، فقره (۱)، جزء ۴',
    descriptionFa: 'اسناد لازم برای کارمندان رسمی مشمول درخواست.',
  },
  {
    key: 'applicantPhotos',
    titleFa: 'عکس درخواست‌دهنده',
    required: true,
    legalBasis: 'ماده ۱۵، فقره (۱)، جزء ۵',
    descriptionFa: 'سه قطعه عکس درخواست‌دهنده.',
    quantity: 3,
  },
  {
    key: 'updatedInitialApplicationInformation',
    titleFa: 'معلومات و مدارک به‌روزشده درخواست اولیه',
    required: false,
    legalBasis: 'ماده ۱۵، فقره (۱)، جزء ۶',
    descriptionFa: 'در صورت بروز تغییرات عمده، معلومات و مدارک ضروری درخواست اولیه باید تجدید شود.',
  },
  {
    key: 'otherDabRequestedInformation',
    titleFa: 'سایر معلومات مورد مطالبه د افغانستان بانک',
    required: false,
    legalBasis: 'ماده ۱۵، فقره (۱)، جزء ۷',
    descriptionFa: 'برای مواردی که د افغانستان بانک حسب لزوم مطالبه می‌کند.',
  },
];

export const DAB_RENEWAL_STATUSES = [
  'draft',
  'documents_pending',
...
export function canSubmitDabRenewal(
  ...
  for (const requirement of DAB_LICENSE_RENEWAL_REQUIREMENTS) {
    ...
    if (!record || record.status !== 'verified') {
      missing.push(requirement.titleFa);
    }
  }
...
/**
 * Article 15 requires the renewal request to be submitted at least three weeks
 * before the license expiry date. This function is intentionally informational:
 * it does not block a case because DAB may apply a different instruction.
 */
export function renewalDeadlineInfo(expiryDate: string, applicationDate: string) {
  ...
  return {
    daysBeforeExpiry,
    meetsThreeWeekRule: daysBeforeExpiry >= minimumDays,
  };
}
